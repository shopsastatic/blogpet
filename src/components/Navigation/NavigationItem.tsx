	...
	const renderMegaMenu = (menu: NavItemType) => {
		...
		return (
			...
			>
				...
				<div className="sub-menu hiddenScrollbar invisible absolute inset-x-0 top-full mt-[15px] z-50 max-h-[70vh] transform overflow-auto">
				...
				{() => (
					<>
						...
						>
							...
							>
								<ul className="relative grid space-y-1 bg-[#1b1b1b] py-3 text-sm shadow-lg ring-1 ring-black ring-opacity-5 text-white">
									{menuDropdown.children?.map(i => {
										if (i.children?.length) {
											...
											return (
												<li
													key={i.id}
													className={`px-2 hover:underline underline-offset-2 ${i.cssClasses?.join(' ')}`}
												>
	...
	const renderMainItem = (item: NavItemType) => {
		...
		return (
			<div className="nc-menu-lv1 flex flex-shrink-0 items-center">
				<ItemC
					className="relative inline-flex cursor-pointer items-center rounded-xl px-4 py-2.5 text-sm font-medium text-neutral-700 xl:px-5 2xl:text-[15px]"
